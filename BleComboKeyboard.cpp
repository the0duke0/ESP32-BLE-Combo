...
static const uint8_t _hid_Keyboard_Media_Mouse_ReportDescriptor[] = {
...
static const uint8_t _hid_Media_ReportDescriptor[] = {
  // ------------------------------------------------- Media Keys
  USAGE_PAGE(1),      0x0C,          // USAGE_PAGE (Consumer)
  USAGE(1),           0x01,          // USAGE (Consumer Control)
  COLLECTION(1),      0x01,          // COLLECTION (Application)
...
void BleComboKeyboard::taskServer(void* pvParameter) {
  ...
  // Load the correct HID descriptor 
  if (bleKeyboardInstance->bleConnectionMode == KEYBOARD_MEDIA_MOUSE_MODE) {
	bleKeyboardInstance->hid->reportMap((uint8_t*)_hid_Keyboard_Media_Mouse_ReportDescriptor, sizeof(_hid_Keyboard_Media_Mouse_ReportDescriptor));
  }
  else if (bleKeyboardInstance->bleConnectionMode == MEDIA_MODE){
    bleKeyboardInstance->hid->reportMap((uint8_t*)_hid_Media_ReportDescriptor, sizeof(_hid_Media_ReportDescriptor));
  }
